package processinggameengine.helpers;

import processinggameengine.gameobjects.CircleObj;
import processinggameengine.gameobjects.GObject;
import processinggameengine.gameobjects.RectObj;

public class Collision {
	public static boolean colPointPoint(int mx, int my, int x, int y) {
		if (Math.sqrt(Math.pow(mx - x, 2) + Math.pow(my - y, 2)) > 0) {
			return true;
		}
		return false;
	}
	...
	public static boolean colPointCircle(GObject a, CircleObj b) {
		if (Math.sqrt(Math.pow(a.getxPos() - b.getxPos(), 2) + Math.pow(a.getyPos() - b.getyPos(), 2)) <= b.getRadius()) {
			return true;
		}
		return false;
	}
	...
	public static boolean colCircleCircle(CircleObj a, CircleObj b) {
		if (Math.sqrt(Math.pow(a.getxPos() - b.getxPos(), 2) + Math.pow(a.getyPos() - b.getyPos(), 2)) <= (a.getRadius() + b.getRadius())) {
			return true;
		}
		return false;
	}
